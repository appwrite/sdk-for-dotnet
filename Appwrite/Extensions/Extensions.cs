...
namespace Appwrite.Extensions
{
    public static class Extensions
    {
        ...
        // Returns an already-materialised sequence on every path that could
        // otherwise be lazy. Arrays are inherently eager; JsonElement arms
        // call ToList() to detach from the originating JsonDocument lifetime;
        // the generic IEnumerable arms call ToList() so callers may safely
        // re-enumerate without depending on single-pass iterator semantics.
        public static IEnumerable<object> ToEnumerable(this object value)
        {
            return value switch
            {
                string => throw new InvalidCastException("Cannot convert string to IEnumerable<object>"),
                JsonElement element when element.ValueKind == JsonValueKind.Array =>
                    element.EnumerateArray().Select(e => (object)e).ToList(),
                JsonElement element =>
                    throw new InvalidCastException($"Cannot convert JsonElement of kind {element.ValueKind} to IEnumerable<object>"),
                object[] array => array,
                IEnumerable<object> enumerable => enumerable.ToList(),
                IEnumerable nonGeneric => nonGeneric.Cast<object>().ToList(),
        ...
        public static List<T> ConvertToList<T>(this object value)
        {
            ...
            {
                return jsonElement.Deserialize<List<T>>(Client.DeserializerOptions)
                    ?? throw new InvalidCastException($"Cannot deserialize {jsonElement} to List<{typeof(T)}>.");
            }

            if (value is IEnumerable<T> typedEnumerable)
            {
            ...
            return value.ToEnumerable()
                ...
                {
                    // Preserve null items as default(T) — for reference and nullable
                    // value types this is null; for non-nullable value types the
                    // caller would have failed earlier deserializing a null array
                    // entry, so this is the least-surprising behavior.
                    null => default!,
                    T typedItem => typedItem,
                    JsonElement jsonItem => jsonItem.Deserialize<T>(Client.DeserializerOptions)!,
                    _ => (T)Convert.ChangeType(item, typeof(T))
                })
                .ToList();
        }
